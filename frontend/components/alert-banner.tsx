...
import { getAlerts, type AdminAlert } from '@/lib/api'
  ...
  useEffect(() => {
    const loadAlerts = async () => {
      try {
        ...
        setAlerts(data.filter((alert) => alert.type === 'global'))
      } catch (err) {
    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex flex-col gap-0">
      {visibleAlerts
        ...
        .map((alert) => {
          ...
          const bgColor = alert.backgroundColor || '#3B82F6'
          const textColor = alert.textColor || '#FFFFFF'
          const borderColor = alert.borderColor || '#1E40AF'
