...
export default function AdminSettings() {
  ...
  async function save() {
    ...
    try {
      ...
      showToast.error(e?.response?.data?.message || e.message || "Failed to save settings");
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="max-w-3xl">
        ...
        <div className="mt-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <div className="h-14 w-14 rounded-xl border border-white/10 bg-white/5 overflow-hidden">
                {form.logoUrl ? (
                  <img src={form.logoUrl} className="h-full w-full object-cover" alt="Logo" />
                ) : (
          ...
          <button
            onClick={save}
            disabled={saving}
            className="rounded-lg bg-sky-500 hover:bg-sky-600 disabled:opacity-60 px-4 py-2 font-semibold transition-colors"
          >
